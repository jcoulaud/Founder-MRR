...
export default function FounderProfile() {
  ...
  useEffect(() => {
    ...
    return () => { document.title = "FounderMRR — Startup Revenue Leaderboard"; };
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="px-6 md:px-8 py-8 md:py-10 flex items-center gap-4 md:gap-5 border-b border-slate-200">
            <div className="flex-1 min-w-0">
              <h1 className="font-display font-black text-2xl md:text-3xl text-slate-900 tracking-tight flex items-center gap-2">
                @{entry.xHandle}
                <a href={`https://x.com/${entry.xHandle}`} target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-slate-900 transition-colors translate-y-px">
            ...
            <div className="flex items-center px-4 py-2.5 rounded-xl bg-gradient-to-b from-amber-50 to-amber-100/50 border border-amber-200">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4">
            {[
              ...
              >
                ...
                <div className={`font-mono text-lg md:text-xl font-bold tabular-nums leading-tight ${stat.color}`}>
                  {stat.value || "\u2014"}
                </div>
              </div>
            ))}
            ...
            <div className="divide-y divide-slate-100">
              {entry.startups.map((startup) => (
                <div key={startup.slug} className="px-6 md:px-8 py-4 flex items-center gap-3.5 hover:bg-slate-50/50 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    {startup.website ? (
                      <a href={startup.website} target="_blank" rel="noopener noreferrer" className="font-semibold text-sm text-slate-900 hover:text-emerald-600 transition-colors">{startup.name}</a>
                    ) : (
